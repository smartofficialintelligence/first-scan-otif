...
-- Positive class for rate features remains long_delivery (>14d duration) — that
-- combination is what the handoff model uses; names stay seller_*_late_rate_*.
with labeled as (
  select
    order_id,
    seller_id,
...
select
  ...
  safe_divide(
    countif(hist_ts >= timestamp_sub(handoff_ts, interval 7 day) and hist_positive = 1),
    nullif(countif(hist_ts >= timestamp_sub(handoff_ts, interval 7 day)), 0)
  ) as seller_late_rate_7d,
  safe_divide(
    countif(hist_ts >= timestamp_sub(handoff_ts, interval 30 day) and hist_positive = 1),
    nullif(countif(hist_ts >= timestamp_sub(handoff_ts, interval 30 day)), 0)
  ) as seller_late_rate_30d,
  safe_divide(
    countif(hist_ts >= timestamp_sub(handoff_ts, interval 90 day) and hist_positive = 1),
    nullif(countif(hist_ts >= timestamp_sub(handoff_ts, interval 90 day)), 0)
