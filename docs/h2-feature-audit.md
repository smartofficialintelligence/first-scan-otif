...
**Status:** completed for portfolio v1 (2026-08-16); **amended 2026-08-18** for ADR 0006 handoff clocks.  
...
Hero decision time is **`handoff_ts`** (first carrier scan). `prediction_ts` is the approval clock for handling / horizon only.
...
| Timestamp / PIT (`event_ts < handoff_ts`) | Pass by design — local `build.py` and dbt `int_seller_history` use `handoff_ts` with left-closed windows |
| Window semantics 7/30/90d | Pass — contract columns match windows |
| Knowable at `handoff_ts` | Pass for request-native + historical aggregates + derived clocks (`handling_days`, `remaining_to_promise_days`, `handling_frac_of_promise`, `limit_miss`) |
| Raw carrier / customer delivery in X | Pass — blocked; only derived clocks allowed |
...
| Explicit non-features excluded | Pass — no review scores / raw delivery timestamps as features |
...
| Amendment | 2026-08-18 — PIT / knowable-at clock is `handoff_ts`; approval `prediction_ts` is not the split or history key |
